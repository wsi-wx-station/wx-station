...
function prepareWeatherData(raw_data,whitelist) {
  // Parse the raw data and strip the unneccessary properties from the data object
  let data = returnWhitelistedDataPoints(JSON.parse(raw_data),whitelist);
  ...
  // set uv value as a small object
  data.uv = uvIndexValues(data.uv);
  // set wind value as a small object
  data.winddir = windValues(data.winddir);
